...
#profiles to plot
prof_ds_id  = np.array([1, 1, 1, 1, 1, 1])
prof_vel_id = np.array([42, 59, 104, 80, 117, 130])
...
vel2plot = np.any([np.logical_and(df_prof_info.DSID==d_id, df_prof_info.VelID==v_id).values 
                   for d_id, v_id in zip(prof_ds_id, prof_vel_id)], axis=0)

#drop outlier profiles
df_prof_info = df_prof_info.loc[vel2plot,:].reset_index(drop=True)
...
for k, vprof_info in df_prof_info.iterrows():
    ...
    ax.legend(handles=hl_emp+hl_usgs+hl_stat+hl_svar, loc='lower right', fontsize=30)
