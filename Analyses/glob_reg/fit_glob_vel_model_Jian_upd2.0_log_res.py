...
#regression info
fname_stan_model = '../stan_lib/jian_fun_glob_reg_upd2.0_log_res.stan'
#iteration samples
n_iter_warmup   = 50000
...
#output directory
dir_out = '../../Data/global_reg/bayesian_fit/JianFunUpd2.0_log_res/' + fname_out_main + '/'
...
# - - - - - - - - - - - 
#hyper-parameters
col_names_hyp = ['r1','r2','s1','s2','s3','sigma_vel']
...
param_k_new   = np.exp( r1_new*np.log(df_velprofs.Vs30.values) + r2_new )
...
param_k_scl   = np.exp( r1_new*np.log(vs30_array) + r2_new )
...
for k, v_id_dsid in enumerate(vel_id_dsid):
    ...
    param_k    = np.exp( r1_new*np.log(vs30) + r2_new)
