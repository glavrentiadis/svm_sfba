...
logVs30mid_fxd = 6.472060
logVs30scl_fxd = 2.870825

#regression info
fname_stan_model = '../stan_lib/jian_fun_glob_reg_upd7.1GPdBr_log_res.stan'
#iteration samples
n_iter_warmup   = 5000
...
#output directory
dir_out = '../../Data/global_reg/bayesian_fit/JianFunUpd7.1GPdBr_log_res/' + fname_out_main + '/'
...
stan_fit = stan_model.sample(data=fname_stan_data, chains=n_chains, 
                             ...
                             show_progress=True, show_console=False, output_dir=dir_out+'stan_fit/')
...
# - - - - - - - - - - - 
#hyper-parameters
col_names_hyp = ['logVs30mid','logVs30scl','r1','r2','s2','sigma_vel','ell_rdB','omega_rdB']
...
# ell_rdB
param_ell_rdB_mu  = np.array([             df_stan_posterior_raw.ell_rdB.mean()]   * n_vel)
param_ell_rdB_med = np.array([             df_stan_posterior_raw.ell_rdB.median()] * n_vel)
param_ell_rdB_sig = np.array([             df_stan_posterior_raw.ell_rdB.std()]    * n_vel)
param_ell_rdB_p16 = np.array([np.quantile( df_stan_posterior_raw.ell_rdB, 0.16)]   * n_vel)
param_ell_rdB_p84 = np.array([np.quantile( df_stan_posterior_raw.ell_rdB, 0.84)]   * n_vel)
# omega_rdB
param_omega_rdB_mu  = np.array([             df_stan_posterior_raw.omega_rdB.mean()]   * n_vel)
param_omega_rdB_med = np.array([             df_stan_posterior_raw.omega_rdB.median()] * n_vel)
param_omega_rdB_sig = np.array([             df_stan_posterior_raw.omega_rdB.std()]    * n_vel)
param_omega_rdB_p16 = np.array([np.quantile( df_stan_posterior_raw.omega_rdB, 0.16)]   * n_vel)
param_omega_rdB_p84 = np.array([np.quantile( df_stan_posterior_raw.omega_rdB, 0.84)]   * n_vel)
#dB_r
...
columns_names = ['param_vs0_mean',    'param_k_mean',    'param_n_mean',
                 ...
                 'ell_rdB_mean',      'ell_rdB_med',     'ell_rdB_std',     'ell_rdB_prc0.16',   'ell_rdB_prc0.84',
                 'omega_rdB_mean',    'omega_rdB_med',   'omega_rdB_std',   'omega_rdB_prc0.16', 'omega_rdB_prc0.84']
...
ax.set_xlim([-3, 3])
ax.set_ylim([0, 500]) if flag_trunc_z1 else ax.set_ylim([0, 200])
ax.invert_yaxis()
...
hl = ax.errorbar(df_params_summary.Vs30, df_params_summary.param_rdB_med,
                 yerr=np.abs(df_params_summary[['param_rdB_prc0.16','param_rdB_prc0.84']].values - df_params_summary.param_rdB_med.values[:,np.newaxis]).T, 
                 capsize=4, fmt='none', ecolor='orangered', label='Posterior Std')
#edit properties
ax.set_xlabel(r'$V_{S30}$ (m/sec)', fontsize=30)
...
for k, v_id_dsid in enumerate(vel_id_dsid):
    ...
    hl2 = ax.plot(vel_m_array,   depth_m_array, linewidth=2.0, linestyle='dashed', label=r'Velocity model (w/o $r_{\delta B}$)')
    hl3 = ax.plot(vel_mdB_array, depth_m_array, linewidth=2.0,                     label=r'Velocity model (w/ $r_{\delta B}$)')
    #edit properties
