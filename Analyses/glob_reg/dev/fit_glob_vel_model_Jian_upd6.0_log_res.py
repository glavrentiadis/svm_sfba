...
#regression info
fname_stan_model = '../stan_lib/jian_fun_glob_reg_upd6.0_log_res.stan'
#iteration samples
n_iter_warmup   = 50000
...
#output directory
dir_out = '../../Data/global_reg/bayesian_fit/JianFunUpd6.0_log_res/' + fname_out_main + '/'
...
#n scaling
s1_new = df_stan_hyp.loc['prc0.50','s1']
s2_new = df_stan_hyp.loc['prc0.50','s2']
s3_new = df_stan_hyp.loc['prc0.50','s3']
#mean profile parameters
param_k_new   = np.exp( r4_new + r3_new * sigmoid((np.log(df_velprofs.Vs30.values)-r1_new)*r2_new) )
...
param_k_scl   = np.exp( r4_new + r3_new * sigmoid((np.log(vs30_array )-r1_new)*r2_new ) )
...
for k, v_id_dsid in enumerate(vel_id_dsid):
    ...
    param_k   = np.exp(  r4_new + r3_new * sigmoid((np.log(vs30)-r1_new)*r2_new) )
