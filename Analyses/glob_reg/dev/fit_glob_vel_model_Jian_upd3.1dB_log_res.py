...
#regression info
fname_stan_model = '../stan_lib/jian_fun_glob_reg_upd3.1dB_log_res.stan'
#iteration samples
# n_iter_warmup   = 50000
...
fname_flatfile = '../../Data/vel_profiles_dataset/Jian_VSPDB_velocity_profles.csv'
...
#output directory
dir_out = '../../Data/global_reg/bayesian_fit/JianFunUpd3.1dB_log_res/' + fname_out_main + '/'
...
# - - - - - - - - - - - 
#hyper-parameters
col_names_hyp = ['r1','s0','s1','s2','s3','sigma_vel','tau_r']
...
#n scaling
s0_new = df_stan_hyp.loc['prc_0.50','s0']
...
param_n_new   = s0_new + s3_new * sigmoid((np.log(df_velprofs.Vs30.values)-s1_new)*s2_new);
...
param_n_scl   = s0_new + s3_new * sigmoid((np.log(vs30_array )-s1_new)*s2_new);
...
for k, v_id_dsid in enumerate(vel_id_dsid):
    ...
    param_n      = s0_new + s3_new * sigmoid((np.log(vs30)-s1_new)*s2_new);
    param_a      =-1/param_n
